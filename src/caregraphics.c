...
void bpm2Text(double bpm, char *text, uint32_t *fontHRColor) {
	if (bpm == -1) {
		...
		*fontHRColor = GRAPHICS_RGBA32(0x00, 0xff, 0xff, 0xff);
...
void *graphicTask(void *arg) {
	...
	while(gr->exit) {
		...
		bpm = gr->heartRate;
		bpm2Text(bpm, text, &fontHRColor);

		// graphics_resource_text_dimensions_ext(img_overlay, )
    	graphics_resource_render_text_ext(img_overlay, 0, 160,
			GRAPHICS_RESOURCE_WIDTH,
			GRAPHICS_RESOURCE_HEIGHT,
			fontHRColor, /* fg */
	  		GRAPHICS_RGBA32(0, 0, 0, 0x00), /* bg */
			text, strlen(text), fontSize);
